...
const { generateStudentCard, generatePayslip, generateTeacherCard, generateMilitaryCard, generateDocumentsParallel, closeBrowser } = require('./services/generator');
...
function generateRealisticBirthDate() {
    ...
    // Fix for Faker v8+: Use object syntax { from, to }
    const fromDate = new Date(`${minYear}-01-01`);
    const toDate = new Date(`${maxYear}-12-31`);

    return faker.date.between({ from: fromDate, to: toDate }).toISOString().split('T')[0];
}
...
function selectBestUniversity(preferUSA = true) {
    if (!UNIVERSITIES || UNIVERSITIES.length === 0) {
        throw new Error('UNIVERSITIES database not loaded');
    }
    ...
    if (candidates.length === 0) return UNIVERSITIES[0]; // Fallback

    // Sort by success rate (universities with no data get priority to test them)
    candidates.sort((a, b) => {
        const statsA = (verificationStats && verificationStats.byUniversity) ? verificationStats.byUniversity[a.name] : null;
        const statsB = (verificationStats && verificationStats.byUniversity) ? verificationStats.byUniversity[b.name] : null;
    ...
    if (topCandidates.length === 0) return candidates[0]; // Fallback if slice fails

    const selected = topCandidates[Math.floor(Math.random() * topCandidates.length)];
    return selected || UNIVERSITIES[0]; // Absolute fallback
}
...
function classifyError(errorResponse) {
    ...
    if (errorIds.includes('fraudRulesReject')) {
        return {
            type: 'fraud_blocked',
            message: '⛔️ SheerID Security Blocked this Request (Fraud Rules).',
            recovery: '💡 Solution: The link is burned. Get a NEW link (incognito mode) and try with a new IP/VPN.'
        };
    }
...
async function verifyStudent(verificationUrl, serviceType = 'spotify') {
    ...
    try {
        ...
        let firstName, lastName, university, email, dob, studentId;

        try {
            // DEBUG LOGS
            if (!faker.person) global.emitLog('⚠️ DEBUG: faker.person is undefined! Checking faker version...');

            firstName = faker.person.firstName();
            lastName = faker.person.lastName();
            global.emitLog(`   🔹 Generated Name: ${firstName} ${lastName}`);

            // IMPROVEMENT: Use weighted university selection based on success rate
            university = selectBestUniversity(true);
            if (!university) throw new Error('selectBestUniversity returned undefined');
            global.emitLog(`   🔹 Selected Uni: ${university.name}`);

            email = faker.internet.email({ firstName, lastName, provider: university.domain || 'psu.edu' });
            global.emitLog(`   🔹 Generated Email: ${email}`);

            // IMPROVEMENT: Use realistic birth date (18-25 years old)
            dob = generateRealisticBirthDate();
            global.emitLog(`   🔹 Generated DOB: ${dob}`);

            studentId = Math.floor(Math.random() * 100000000).toString();

        } catch (err) {
            global.emitLog(`❌ DEBUG ERROR in Identity Gen: ${err.message}`);
            global.emitLog(err.stack);
            throw err;
        }
